...
def load_checkpoint(model, checkpoint, model_only=False):
...
if __name__ == "__main__":
    ...
    nerf_fine = NeRFNetwork()
    load_checkpoint(nerf_fine, args.ckpt_path)
    nerf_fine.cuda().eval()
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(0, B, args.chunk)):
            ...
            sigma, color = [nerf_fine(xyz_embedded, dir_embedded)]
            out_chunks += [torch.cat([rgb, sigma], -1)]
